...
function Details() {
  ...
  const languageArray = dataObj?.languages
    ? Object.values(dataObj.languages)
    : [];
  const currencyArray = dataObj?.currencies
    ? Object.keys(dataObj.currencies)
    : [];
    ...
    <section className="detailsPage-wrapper">
      <div className={`detailsPage-content ${!isDark && "light-card-bg"}`}>
        ...
        <header className="detailsPage-header">
          ...
          <div className={`population-area ${!isDark && "light-text"}`}>
            <div className={`population-wrapper ${!isDark && "light-body-bg"}`}>
              ...
              <span className={`divider ${!isDark && "light-divider"}`}></span>
              <p className={`value ${!isDark && "light-text"}`}>
        ...
        <ul className="main-details-wrapper">
          <li className="item">
            Capital{" "}
            <span className={`value ${!isDark && "light-text"}`}>
              {dataObj?.capital?.[0]}
            </span>
          </li>
          <li className="item">
          ...
          <li className="item last">
            Continents{" "}
            <span className={`value ${!isDark && "light-text"}`}>
              {dataObj?.continents?.[0]}
            </span>
          </li>
        </ul>
        {/* Neighboring Countries content */}
        <div className="borders-wrapper">
          <h3 className={`border-heading ${!isDark && "light-text"}`}>Neighboring Countries</h3>
          <ul className="borders-list">
            {neighborsArray.map((border) => (
              <li key={border?.cca3} className={`border-item ${!isDark && "light-text"}`}>
